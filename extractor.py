...
class ContentXExtractor:
    ...
    async def get_m3u8_link(self, url, referer):
        ...
        print(f"  -> ContentX Extractor çalışıyor: {url}", flush=True)
        ...
        if dublaj_match:
            try:
                ...
                print(f"     Dublaj kaynağı çekiliyor: {dublaj_source2_url}", flush=True)
                ...
                print(f"     Dublaj linki işlenirken hata: {e}", flush=True)
